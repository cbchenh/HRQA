...
function [IdxM, HRR, HMean, HVar, HSkew, HKurt, HEnt, HGini] = RHRQA(s, K, r, a, B)
    ...
    if nargin < 5
        if nargin < 4
           ...
           a = 0.9999*sin(pi/K)/(1+sin(pi/K)); % set the default a as close as to the boundry
        end
        B=20;
    end
    if K < max(s)
    ...
    if(r == 0)
        ...
        HRR = sum((H_bar./N).^2);
        LC_D = (1/a)^(r+1)*pdist(Cv(Idx_set,:)); % Regularization is based on a and r 
        HMean = mean(LC_D);
